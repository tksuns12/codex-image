...
fn install_uat_docs_readme_covers_install_usage_and_codex_backend() {
    ...
    assert_all_present(
        ...
        &[
            "cargo install --path .",
            "raw.githubusercontent.com/tksuns12/codex-image/release/scripts/install-latest.sh",
            "raw.githubusercontent.com/tksuns12/codex-image/release/scripts/install-latest.ps1",
    ...
    assert_all_present(
        ...
        &[
            ...
            "## Install",
            "verifies the downloaded archive against `SHA256SUMS`",
            "codex-image generate \"A watercolor fox reading in a library\" --out ./out",
            "image-0001.<format>",
            "manifest.json",
            "human-readable success line on stdout",
            "manifest.json` is always written under `--out`",
...
fn install_uat_docs_advanced_reference_covers_post_first_run_operations() {
    ...
    assert_all_present(
        ...
        &[
            ...
            "stale root `manifest.json` is removed before batch work starts",
            "completed item directories remain for inspection",
            "positive local Codex subprocess timeout",
            "not forwarded to Codex",
            "current-run artifacts",
            "predates current invocation",
            "response_contract.image_generation",
            "`--debug-diagnostics <FILE>`",
            "schema version 1",
            "safe modes, counts, statuses, and placeholders",
            "no raw prompts, prompt hashes, raw stdout/stderr, token sentinels, final-message payloads, credentials, sensitive paths, image payloads, or exact Codex environment sentinel",
...
fn install_uat_docs_korean_readme_covers_install_usage_and_codex_backend() {
    ...
    assert_all_present(
        ...
        &[
            ...
            "manifest.json",
            "사람이 읽기 쉬운 성공 메시지",
            "manifest.json`은 항상 `--out` 디렉터리에 기록됩니다.",
